    ...
    def process_item(self, item,   spider):
        if (spider.name == 'getlink'):
            try:
                ...
                sql = """INSERT IGNORE INTO dish_material(link) VALUES (\'%s\')""" %(''.join(item['link']).decode('utf-8'))
            ...
            try:
            	# sua lenh update
                self.cursor.execute("UPDATE dish_material SET material = %s, value = %i, type = %s WHERE link = %s",(''.join(item['material']), ''.join(item['value']), ''.join(item['type']),''.join(item['link'])))
                self.conn.commit()
